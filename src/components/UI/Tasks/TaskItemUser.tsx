...
const statusMap = {
  "No Assignee": { text: "No Assignee", color: "bg-gray-200 text-gray-700", icon: FaPlusCircle },
  "In Progress": { text: "In Progress", color: "bg-yellow-200 text-yellow-800", icon: FaSpinner },
  "Completed": { text: "Completed", color: "bg-green-200 text-green-800", icon: FaCheckCircle },
};
...
export default function EmployeeTaskTable({ tasks }: EmployeeTaskTableProps) {
  ...
  if (!username) return <p className="text-center text-gray-500 p-4">กรุณา login ก่อน</p>;
  if (visibleTasks.length === 0) return <p className="text-center text-gray-500 p-4">ไม่มีงานที่ได้รับมอบหมาย</p>;
      ...
      <table className="min-w-full divide-y divide-gray-200">
        <thead className="bg-gray-50">
          <tr>
            ...
            <th className="px-4 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Assignees</th>
            <th className="px-4 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Due Date</th>
            <th className="px-4 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Updated At</th>
        ...
        <tbody className="bg-white divide-y divide-gray-200">
          {visibleTasks.map((task) => {
            ...
            const statusInfo = statusMap[task.status] || statusMap["No Assignee"];

            return (
              <tr key={task.id} className="hover:bg-gray-50 transition-colors duration-200 odd:bg-white even:bg-gray-50">
                  ...
                  {isAssignedToMe && (
                    <div className="flex justify-center gap-2">
                      <Button
                        ...
                        className={`font-semibold ${task.status === "Completed" ? "bg-blue-500 hover:bg-blue-600" : "bg-green-500 hover:bg-green-600"} text-white px-3 py-1.5 text-xs rounded-md transition-colors`}
                      />
